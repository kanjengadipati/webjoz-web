...
export default function BenefitsStatGrid({ benefits: b }: BenefitsVariantProps) {
      ...
      <div style={{ maxWidth: "72rem", margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(1, 1fr)", gap: "1.5rem" }} className="sm:grid-cols-2 lg:grid-cols-3">
          {b.items?.map((item, idx) => (
            <div key={idx} style={{ background: "var(--dt-surface)", border: `1px solid color-mix(in srgb, var(--dt-primary) 15%, transparent)`, borderTop: `3px solid var(--dt-primary)`, borderRadius: "var(--dt-radius-lg)", padding: "2rem", display: "flex", flexDirection: "column", gap: "0.75rem", boxShadow: `0 4px 12px color-mix(in srgb, var(--dt-primary) 10%, transparent)`, transition: "box-shadow 0.2s, transform 0.2s" }}
