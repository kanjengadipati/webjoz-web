      ...
      {stats.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1.5rem", marginBottom: "3rem" }}>
          {stats.map((stat, i) => {
            ...
            return (
              <div key={i} style={{ background: "var(--dt-surface)", border: `1px solid color-mix(in srgb, var(--dt-primary) 15%, transparent)`, borderRadius: "var(--dt-radius-lg)", padding: "1.5rem 1.25rem", position: "relative", overflow: "hidden" }}>
                ...
                <p style={{ fontSize: "clamp(1.75rem, 4cqw, 2.75rem)", fontWeight: 800, color: "var(--dt-text)", margin: 0, lineHeight: 1.1, fontFamily: "var(--dt-heading-font)" }}>{stat.value}</p>
                <p style={{ fontSize: "0.85rem", color: "var(--dt-text-muted)", margin: "0.25rem 0 0 0", textTransform: "uppercase", letterSpacing: "0.05em" }}>{stat.label}</p>
              </div>
            );
          })}
        </div>
